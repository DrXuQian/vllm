...
__global__ void reshape_and_cache_kvfloat13_bf16_kernel(
  ...
  const int64_t head_idx = blockIdx.y * warps_per_block + warp_id;

  if (head_idx >= num_heads) {
    return;
  }
  const int pack_thread = lane & (kKVFloat13PackThreads - 1);
  const bool is_value = lane >= kKVFloat13PackThreads;
  ...
  const uint16_t* src_base =
      (is_value ? value : key) +
      token_idx * (is_value ? value_tok_stride : key_tok_stride) +
      head_idx * (is_value ? value_head_stride : key_head_stride);
  uint8_t* dst_base =
      kv_cache + (is_value ? cache_kv_stride : 0) +
  ...
  for (int64_t chunk_idx = 0; chunk_idx < num_chunks; ++chunk_idx) {
    const uint16_t* src = src_base + chunk_idx * kKVFloat13ChunkSize;
    uint8_t* dst = dst_base + chunk_idx * kKVFloat13ChunkBytes;
...
#pragma unroll
    for (int i = 0; i < 8; ++i) {
      ...
      if ((i & 1) == 0) {
        ...
        exp_hi_packed |=
            static_cast<uint32_t>(exp_hi4) << (((i >> 1) * 8) + 4);
      }
      em_packed |= static_cast<uint64_t>(((exp5 & 1) << 7) | mant7)
...
void reshape_and_cache_kvfloat13(torch::Tensor key,
  ...
  const int64_t warps_per_block =
      std::min<int64_t>(num_heads, kKVFloat13MaxWarpsPerBlock);
  const dim3 grid(
      num_tokens,
